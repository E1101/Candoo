<?php
class cLocali_Lib_Date extends Zend_Date_DateObject
{
	...
	// a duplicate of $_locale to access from static methods
	protected static $locale;
	
	protected static $_calendar = null;

	/**
	 * Generates the standard date object, could be a unix timestamp, cLocalized date,
	 ...
	 * 													!!!! format haaye date php nist
	 * 													format haaye hade aksar 5 characteri e const haaye 
	 * 													class ast exp. TTTTT
	 * 
	...
	{
	    // Hengaami ke locale be onvaan e parameter e aval be __construct ersaal mishavad
	    // exp. new Zend_Date( (obj) $locale ) parameter haaye ba`di naadide gerefte mishavad
		if (is_object($date) and !($date instanceof Zend_TimeSync_Protocol) and !($date instanceof Zend_Date)) {
			/**
			 * @fixme change $locale to $date
			 */
		    if (/* $locale */ $date instanceof Zend_Locale) {
				$locale = $date;
				$date   = null;
				$part   = null;
			} else {
		...
		if (($date !== null) and !is_array($date) and !($date instanceof Zend_TimeSync_Protocol) and
			...
			$date   = null;
			$part   = null;
		// exp. new Zend_Date(time(),'fa_IR');
		} else if (($part !== null) and !defined($part) and Zend_Locale::isLocale($part, true, false)) {
			$locale = $part;
			$part   = null;
		}
		
		// agar locale be shive haaye baalaa be class ersaal nashode bood 
		if ($locale == null) {
		...
		// exp. new Zend_Date(Zend_Date::DATE_FULL); part haa bishtar az 5 character nist
		...
		if ($date === null) {
			...
			if ($part === null) {
				$date = time();
			// exp. new Zend_Date(null,Zend_Date::DATE_FULL);
			} else if ($part !== self::TIMESTAMP) {
				$date = self::now($locale);
				// get date as $part format like "Monday, April 23, 2012"s
				$date = $date->get($part);
			}
		}
		...
		// exp. new Zend_Date(Zend_Date::now()[,Zend_Date::DATE_FULL]);
		// 	    !!!!!! new Zend_Date('Asia/Kolkata'[,Zend_Date::DATE_FULL]);
		// 		hengaami ke string ersaal mishavad error daarim hengaame $this->set($date
		//      $date baayad object baashad yaa array
		if (!is_int($date)) {
		...
		if (($part !== null && $part !== self::TIMESTAMP) or (!is_numeric($date))) {
			...
			/**
			 * @fixme $date dar $this->_calculate('set',$date
			 * 		  nemitavaanad string baashad
			 */
	...
	public function toString($format = null, $type = null, $locale = null)
	{
		...
		if ($format === null) {
		     ...
		     * Edited by myself ````````````````````````````````````````````````````````
		     */
			$format = self::getDefaultDateFormat() . ' ' . self::getDefaultTimeFormat();
			/** ```````````````````````````````````````````````````````````````````````` */
	...
	{	    
	    ...
	    $calObject = 'cLocali_Lib_Date_Calendar_'.ucfirst(strtolower($calendar));
	    
	    if (class_exists($calObject)) 
	    {
	        $date = new $calObject($format,$timestamp);
	 ...
	 * This could be for example a cLocalized monthname, the time without date,
	...
	private function _parseIsoToDate($token, $locale) {	    
		switch($token) {
			...
			case self::WEEKDAY_SHORT :
				...
				$day     = Zend_Locale_Data::getContent($locale, 'day', array(self::getCalendar(), 'format', 'wide', $weekday));
			...
			case self::WEEKDAY :
				$weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
				return $this->_toComment(Zend_Locale_Data::getContent($locale, 'day', array(self::getCalendar(), 'format', 'wide', $weekday)));
				break;
			...
			/* baraaie inke cLocalize besheh @see WEEKDAY_NARROW
			...
			case 'EEEEE' :
				$weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
				$day = Zend_Locale_Data::getContent($locale, 'day', array(self::getCalendar(), 'format', 'abbreviated', $weekday));
				return $this->_toComment(iconv_substr($day, 0, 1, 'UTF-8'));
				break;
	
			case self::WEEKDAY_NAME :
				$weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
				return $this->_toComment(Zend_Locale_Data::getContent($locale, 'day', array(self::getCalendar(), 'format', 'abbreviated', $weekday)));
				break;
			...
			case self::MONTH_NAME :
				...
				return $this->_toComment(Zend_Locale_Data::getContent($locale, 'month', array(self::getCalendar(), 'format', 'wide', $month)));
				break;
			...
			case self::MONTH_NAME_SHORT :
				$month = $this->date('n', $this->getUnixTimestamp(), false);
				return $this->_toComment(Zend_Locale_Data::getContent($locale, 'month', array(self::getCalendar(), 'format', 'abbreviated', $month)));
				break;
			...
			case self::MONTH_NAME_NARROW :
				$month = $this->date('n', $this->getUnixTimestamp(), false);
				$mon = Zend_Locale_Data::getContent($locale, 'month', array(self::getCalendar(), 'format', 'abbreviated', $month));
			...
			case 'GGG' :
				...
				$return = '';
				if ($year < 0) {
					$return = Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Abbr', '0'));
				}
				if ($return != '') {
				    return $this->_toComment($return);
				}
	
				$return =  Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Abbr', '1'));
				
				return ($return!='') ? $this->_toComment($return) : $this->_toComment(Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Abbr', '0')));
				break;
			...
			case self::ERA_NARROW :
				...
				if ($return != '') {
					return $this->_toComment(iconv_substr($return, 0, 1, 'UTF-8')) . '.';
				}
	
				$return =  Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Abbr', '1'));
				return ($return!='') ? $this->_toComment(iconv_substr($return, 0, 1, 'UTF-8')) . '.' : $this->_toComment(iconv_substr(Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Abbr', '0')),0, 1, 'UTF-8')) . '.';
				break;
			...
			case self::ERA_NAME :
				...
				$return = '';
				if ($year < 0) {
					$return = Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Names', '0'));
				}
				if ($return != '') {
				    return $this->_toComment($return);
				}
	
				$return =  Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Names', '1'));
				
				return ($return!='') ? $this->_toComment($return) : $this->_toComment(Zend_Locale_Data::getContent($locale, 'era', array(self::getCalendar(), 'Names', '0')));
				break;
	
			case self::DATES :
				return $this->_toToken(self::getDefaultDateFormat(), $locale);
				break;
	
			case self::DATE_FULL :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'full')), $locale);
				break;
	
			case self::DATE_LONG :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'long')), $locale);
				break;
	
			case self::DATE_MEDIUM :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'medium')), $locale);
				break;
	
			case self::DATE_SHORT :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'short')), $locale);
				break;
			...
			case self::DATETIME :
				return $this->_toToken(self::getDefaultDateFormat().' '.self::getDefaultTimeFormat(), $locale);
				break;
	
			case self::DATETIME_FULL :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'full')), $locale);
				break;
	
			case self::DATETIME_LONG :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'long')), $locale);
				break;
	
			case self::DATETIME_MEDIUM :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'medium')), $locale);
				break;
	
			case self::DATETIME_SHORT :
				return $this->_toToken(Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'short')), $locale);
				break;
	
			case self::ATOM :
				return $this->_toToken('Y-m-dTH:i:sP',$locale);
				break;
	
			case self::COOKIE :
				return $this->_toToken('l, d-M-y H:i:s e',$locale);
				break;
	
			case self::RFC_822 :
				return $this->_toToken('D, d M y H:i:s O',$locale);
				break;
		...
		// things that minning to date function and won`t to commented
		$mean = array('i','P','O','o');
		if (in_array($token, $mean)) {
		    return $token;
		}
	 ...
	 * This could be for example a cLocalized dayname, the date without time,
	...
	private function _calculate($calc, $date, $part, $locale)
	{
		...
		switch($part) {
			...
			case self::WEEKDAY_NARROW:
				$daylist = Zend_Locale_Data::getList($locale, 'day', array(self::getCalendar(), 'format', 'abbreviated'));
				$weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso', $locale);
				$cnt = 0;
				foreach ($daylist as $key => $value) {
			...
			case self::MONTH_NAME_SHORT:
				$monthlist = Zend_Locale_Data::getList($locale, 'month', array(self::getCalendar(), 'format', 'abbreviated'));
				$cnt = 0;
				foreach ($monthlist as $key => $value) {
					if (strtoupper($value) == strtoupper($date)) {
						$found = $key;
			...
			case self::MONTH_NAME_NARROW:
				$monthlist = Zend_Locale_Data::getList($locale, 'month', array(self::getCalendar(), 'stand-alone', 'narrow'));
				$cnt       = 0;
				foreach ($monthlist as $key => $value) {
					if (strtoupper($value) === strtoupper($date)) {
						$found = $key;
			...
			case self::DATE_FULL:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'full'));
			...
			case self::DATE_LONG:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'long'));
					$parsed = Zend_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
			...
			case self::DATE_MEDIUM:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'medium'));
					$parsed = Zend_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
			...
			case self::DATE_SHORT:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'short'));
					$parsed = Zend_Locale_Format::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
			...
			case self::TIME_FULL:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'full'));
			...
			case self::TIME_LONG:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'long'));
					$parsed = Zend_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
					if ($calc != 'set') {
						$month = 1;
						$day   = 1;
			...
			case self::TIME_MEDIUM:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'medium'));
					$parsed = Zend_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
					if ($calc != 'set') {
						$month = 1;
						$day   = 1;
			...
			case self::TIME_SHORT:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'short'));
					$parsed = Zend_Locale_Format::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
					if ($calc != 'set') {
						$month = 1;
						$day   = 1;
			...
			case self::DATETIME_FULL:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'full'));
			...
			case self::DATETIME_LONG:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'long'));
					$parsed = Zend_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
			...
			case self::DATETIME_MEDIUM:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'medium'));
					$parsed = Zend_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
					if (($calc == 'set') || ($calc == 'cmp')) {
						--$parsed['month'];
						--$month;
			...
			case self::DATETIME_SHORT:
				try {
					$format = Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'short'));
					$parsed = Zend_Locale_Format::getDateTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
	...
	private function _month($calc, $month, $locale)
	{
		...
		if ($month instanceof Zend_Date) {
			...
			if (is_numeric($month)) {
				...
				$monthlist  = Zend_Locale_Data::getList($locale, 'month');
				$monthlist2 = Zend_Locale_Data::getList($locale, 'month', array(self::getCalendar(), 'format', 'abbreviated'));
	 ...
	 * If a cLocalized monthname is given it will be parsed with the default locale or the optional
	 * set locale.
	 * Returned is the new date object
	 *
	 ...
	 * If a cLocalized dayname is given it will be parsed with the default locale or the optional
	 * set locale.
	 * Returned is the new date object
	...
	/**
	 * Sets a new weekday
	 * The weekday can be a number or a string. If a cLocalized weekday name is given,
    ...
    public function setLocale($locale = null)
    {
        try {
            ...
            /**
             * edited by me baraaie inke mikhaam method haaie static bedoone 
             * ijaad e instance az class be locale dastresi daashte baashand
             */
            self::$locale  = $this->_locale;
        } catch (Zend_Locale_Exception $e) {
            require_once 'Zend/Date/Exception.php';
            throw new Zend_Date_Exception($e->getMessage(), 0, $e);
        }
    ...
    public static function isDate($date, $format = null, $locale = null)
    {
    	...
    	$format = self::_getcLocalizedToken($format, $locale);
    	if (!is_array($date)) {
    		try {
    ...
    /**
     * Returns the ISO Token for all cLocalized constants
     *
     * @param string $token Token to normalize
     * @param string $locale Locale to search
     * @return string
     */
    protected static function _getcLocalizedToken($token, $locale)
    {
    	switch($token) {
    		case self::ISO_8601 :
    		...
    		case self::DATES :
    			...
    			break;
    		case self::DATE_FULL :
    			return Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'full'));
    			break;
    		case self::DATE_LONG :
    			return Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'long'));
    			break;
    		case self::DATE_MEDIUM :
    			return Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'medium'));
    			break;
    		case self::DATE_SHORT :
    			return Zend_Locale_Data::getContent($locale, 'date', array(self::getCalendar(), 'short'));
    			break;
    		case self::TIMES :
    			return Zend_Locale_Data::getContent($locale, 'time');
    			break;
    		case self::TIME_FULL :
    			return Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'full'));
    			break;
    		case self::TIME_LONG :
    			return Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'long'));
    			break;
    		case self::TIME_MEDIUM :
    			return Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'medium'));
    			break;
    		case self::TIME_SHORT :
    			return Zend_Locale_Data::getContent($locale, 'time', array(self::getCalendar(), 'short'));
    			break;
    		case self::DATETIME :
    			return Zend_Locale_Data::getContent($locale, 'datetime');
    			break;
    		case self::DATETIME_FULL :
    			return Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'full'));
    			break;
    		case self::DATETIME_LONG :
    			return Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'long'));
    			break;
    		case self::DATETIME_MEDIUM :
    			return Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'medium'));
    			break;
    		case self::DATETIME_SHORT :
    			return Zend_Locale_Data::getContent($locale, 'datetime', array(self::getCalendar(), 'short'));
    			break;
    		case self::ATOM :
    		case self::RFC_3339 :
    		case self::W3C :
	...
	/**
	 * Taghvime rasmi ke tavasote yek locale estefaade mishavad raa bar migardaanad
	 * ma`moolan baraaie hameie locale haa gregorian raa bar migardaanad,
	 * magar dar file e data e locale edit shavad
	 * /Lib/Zend/Locale/Data/[fa].xml
	 * <dates>
	 <dateRangePattern>{0} تا {1}</dateRangePattern>
	 <calendars>
	 <!-- I have to add default calendar type manualy -->
	 <default choice="persian" />
	 *
	 * @param string $locale
	 */
	public static function getCalendar($locale = null)
	{
	    ...
	    $calendar = Zend_Locale_Data::getContent($locale, 'defaultcalendar');
	    self::$_calendar = $calendar;
	    
		return self::$_calendar; 
	}
	
	public static function setCalendar($calendar)
	{
	    $calendar = strtolower($calendar);
	    
	    if (!self::isCalendar($calendar)) {
	        throw new Exception('Calendar '.$calendar.' not valid.');
	    }
	...
	{
	    $locale = self::$locale;
	    return Zend_Locale_Data::getList($locale, 'type','calendar');
	}
	
	public static function isCalendar($calend)
	{
	    $calend = strtolower($calend);
	    
	    $locale = self::$locale;
	    $list   =  Zend_Locale_Data::getList($locale, 'type','calendar');
	    
	    return array_key_exists($calend, $list);
	}
	
	/**
	 * @return string هجری شمسی
	 */
	/* public static function getDateEraLong()
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    return Zend_Locale_Data::getContent($locale, 'era',array($calend,'Names',1));
	}
	 
	public static function getDateEraShort()
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    return Zend_Locale_Data::getContent($locale, 'era',array($calend,'Abbr',1));
	}
	*/
	 ...
	 * array(4) {
  		["minDays"] => string(1) "4"
  		["firstDay"] => string(3) "sat"
  		["weekendStart"] => string(3) "thu"
  		["weekendEnd"] => string(3) "fri"
	   }
	 */
	public function getWeekList()
	{
	    ...
	    return Zend_Locale_Data::getList($locale, 'week', $calend);
	}
	
	public static function getDateAmPmTranslation($ampm)
	{
	    $ampm = strtolower($ampm);
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    return Zend_Locale_Data::getContent($locale, $ampm, $calend);
	}
	
	/**
	 * 
	 * @param string $field => "era" "year" "month" "week" "day" "weekday" "hour" "minute" "second" "zone"
	 * @return string exp 'year' = 'سال'
	 */
	public static function getDatefieldTranslation($field) 
	{
	    $field  = strtolower($field); 
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    $list = Zend_Locale_Data::getList($locale, 'field','gregorian');
	    	    
	    return (isset($list[$field])) ? $list[$field] : null;
	}
	
	public static function getDatetimeFormats($type = null)
	{
		$locale = self::$locale;
		$calend = self::getCalendar();
			
		$list = Zend_Locale_Data::getList($locale, 'datetime', $calend);
		if ($type) {
	...
	{
	    return self::getTimeFormats(self::getDefaultTimeFormatName());
	}
	public static function getDefaultTimeFormatName()
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    return Zend_Locale_Data::getContent($locale, 'defaulttime', $calend);
	}
	public static function getTimeFormats($type = null)
	{
		$locale = self::$locale;
		$calend = self::getCalendar();
		 
		$list = Zend_Locale_Data::getList($locale, 'time', $calend);
		if ($type) {
			return (isset($list[$type])) ? $list[$type] : null;
		}
	...
	{
		return self::getDateFormats(self::getDefaultDateFormatName());
	}
	public static function getDefaultDateFormatName()
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    return Zend_Locale_Data::getContent($locale, 'defaultdate',$calend);
	}
	public static function getDateFormats($type = null)
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    $list = Zend_Locale_Data::getList($locale, 'date', $calend);
	    if ($type) {
	        return (isset($list[$type])) ? $list[$type] : null;
	    } 
	...
	// Days ````````````````````````````````````````````````````````````````````````````````````````````````````````|
	public static function getLocaleDays($format=null)
	{
		$locale = self::$locale;
		$calend = self::getCalendar();
		 
		if ($format == null) {
			$format = Zend_Locale_Data::getContent($locale, 'defaultday',$calend);
		}
	
		return Zend_Locale_Data::getList($locale, 'day',array($calend,'format',$format));
	}
	 ...
	 * @return array(12) {
  		[1] => string(14) "فروردین"
  		[2] => string(16) "اردیبهشت"
  		[3] => string(10) "خرداد"
  		
	 * @param string $format can be wide | narrow
	 */
	public static function getLocaleMonths($format=null)
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    
	    if ($format == null) {
	        $format = Zend_Locale_Data::getContent($locale, 'defaultmonth',$calend);
	    }
	  	    
	    return Zend_Locale_Data::getList($locale, 'month',array($calend,'format',$format));
	}
	
	public static function getMonthName($monthNumber,$format = null) 
	{
	    $locale = self::$locale;
	    $calend = self::getCalendar();
	    ...
	    $months = Zend_Locale_Data::getList($locale, 'month',array($calend,'format',$format));
	...
	// Time Zones ``````````````````````````````````````````````````````````````````````````````````````````````````|
	/**
	 * Yek array shaamele tamaamie time zone haa bar migardaanad
	 * array(223) {
  		["Etc/Unknown"] => string(12) "نامشخص"
  		["Europe/Andorra"] => string(12) "آندورا"
  		["Asia/Kabul"] => string(8) "کابل"
	 	...
	 */
	public static function getTimezoneCities()
	{
		$locale = self::$locale;
		 
		$timeZones = Zend_Locale_Data::getList($locale, 'citytotimezone');
		return $timeZones;
	}
	 ...
	 * Yek array shaamele tamaamie time zone haa bar migardaanad
	 * array(223) {
  		[0] => string(11) "Etc/Unknown"
  		[1] => string(14) "Europe/Andorra"
  		[2] => string(10) "Asia/Kabul"
  		...
	 */
	public static function getTimezones()
	{
	    ...
	    $timeZones = array_keys(Zend_Locale_Data::getList($locale, 'citytotimezone'));
	    return $timeZones;
	}
	
	/**
	 * Name shahre yek timezone raaa bar migardaanad
	 * Asia/Tehran = تهران
	 * 
	 * @param string $timeZone
	 */
	public static function getTimezoneToCity($timeZone)
	{
	    $timeZones = self::getTimezoneCities();
	...
	/**
	 * Name timezone raaa bar asaase yek shahr bar migardaanad
	 *
	 * @param string $city
	 */
	public static function getCityToTimezone($city)
	{
		$timeZones = array_flip(self::getTimezoneCities());

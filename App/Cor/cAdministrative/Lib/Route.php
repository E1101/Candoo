<?php
class cAdministrative_Lib_Route extends Zend_Controller_Router_Route_Abstract
{
    /**
     * URI delimiter
     */
    ...
    public function match($request)
    {
        ...
        // momken ast az aanjaaii ke query haaie admin samte code mishavand
        // yek query e ke tebghe system encode nashode vaared shavad
        // exp. /admin/sdfsf/sdfsf
        // pas meghdaar e baazgashti momken ast yechizi shabihe s^IqÁ¹‘ baashad
        $query  = $this->_decodeQuery($query); 
        
        // test e query raa gharaar nadaadam chon miskham dar har soorat dashboard
        // neshaan daade shavad
    ...
    public function assemble($data = array(), $reset = false, $encode = true)
    {
        ...
        $url = $this->_encodeQuery(http_build_query($params));
    ...
    protected function _decodeQuery($query)
    {
        ...
        Util_Datasecurity_Ancryptst::setKey($key);
        
        return urldecode(Util_Datasecurity_Ancryptst::decrypt($query));
    }
    ...
    protected function _encodeQuery($query)
    {
        ...
        return Util_Datasecurity_Ancryptst::encrypt(urlencode($query));
    }
    ...
    {
        $site = Candoo_App_Resource::get('request')->getSitename();
        // subsite haaa ham baayad az session e marboot be khod estefaade konand
        $sessNamespace = 'SESSION_NAMESPACE_ADMIN_ROUTE_'.Util_String::makeSafe($site);
        $session       = new Zend_Session_Namespace($sessNamespace);
        
        if (! $session->key_pass) {
        	$session->key_pass = Util_String::makeRandStr(8);    
        }
                
        return $session->key_pass;
    }
